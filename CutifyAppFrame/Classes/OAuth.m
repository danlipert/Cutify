 ...
 */
- (void) synchronousRequestToken:(NSString *)requestUrl WithCallbackUrl:(NSString *)callbackUrl {

	NSLog(@"About to request request token at %@ with callback %@", requestUrl, callbackUrl);
	...
	NSString *oauth_header = [self oAuthHeaderForMethod:@"POST" andUrl:requestUrl andParams:params];
 ...
 */
- (void) synchronousAuthorizeToken:(NSString *)authorizeUrl WithVerifier:(NSString *)oauth_verifier {
	
	NSLog(@"About to request access token at %@ with verifier %@", authorizeUrl, oauth_verifier);
	...
	NSString *oauth_header = [self oAuthHeaderForMethod:@"POST" andUrl:authorizeUrl andParams:params andTokenSecret:oauth_token_secret];

	ASIHTTPRequest *request = [ASIHTTPRequest requestWithURL:[NSURL URLWithString:authorizeUrl]];
	request.requestMethod = @"POST";
	[request addRequestHeader:@"Authorization" value:oauth_header];
	[request startSynchronous];
